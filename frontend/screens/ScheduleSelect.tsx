...
interface ScheduleSelectProps {
  onBack: () => void;
  onNext: (date: string, startTime: string, endTime: string) => void;
...
export const ScheduleSelect: React.FC<ScheduleSelectProps> = ({ onBack, onNext, selectedRoom, userRole }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedRoom || !date || !startTime || !endTime || !purpose) return;
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-between max-w-2xl mx-auto">
            <div className="flex flex-col items-center">
              <div className="w-8 h-8 rounded-full bg-brand text-white flex items-center justify-center font-bold text-sm ring-4 ring-white z-10 cursor-pointer" onClick={onBack}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRoom.amenities.map(a => (
                    <span key={a} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gray-100 text-xs text-gray-600">{a}</span>
                  ))}
                </div>
              </div>
              <div className="pt-2">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="flex justify-end gap-4 pt-4 border-t border-gray-100">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-8 py-2.5 rounded-lg bg-brand text-white font-semibold hover:bg-blue-700 shadow-lg shadow-blue-500/30 flex items-center gap-2 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {loading ? (
